...
// Cliente Gemini só será criado no servidor
...
export class GeminiService {
  ...
  /**
   * Executa uma automação usando Google Gemini
   */
  async executeAutomation(request: AutomationRequest): Promise<AutomationResponse> {
    try {
      ...
      console.log('Tentando conectar com Google Gemini...')
      console.log('API Key length:', process.env.GEMINI_API_KEY?.length)

      const client = getGeminiClient()
      if (!client) {
        throw new Error('Cliente Gemini não disponível no browser')
      }
      ...
      console.log('Resposta do Gemini:', text ? 'Sucesso' : 'Vazia')
      ...
      console.error('Erro detalhado na automação Gemini:', error)
...
export function useGemini() {
  const executeAutomation = async (request: AutomationRequest): Promise<AutomationResponse> => {
    try {
      console.log('Enviando requisição para API Gemini:', request.type)
      
      const response = await fetch('/api/gemini/automate', {
        method: 'POST',
        headers: {
      ...
      console.log('Resposta da API Gemini:', result.success ? 'Sucesso' : 'Erro')
      
      return result
    } catch (error) {
      console.error('Erro na comunicação com API Gemini:', error)
      return {
        success: false,
